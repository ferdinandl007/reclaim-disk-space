...
printf 'workers\telapsed_seconds\tmetadata_entries\tpeak_rss_bytes\tpermission_errors\tpartial_directories\n'
for worker in $WORKERS; do
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$worker" "$(field elapsed_seconds)" "$(field metadata_entries)" "${rss:-unknown}" "$(field permission_errors)" "$(field partial_directories)"
done
